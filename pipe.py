...
class PipeHandler:
    def __init__(self,game):
        self.game = game
        self.pipe_distance = DISTANCE_BETWEEN_PIPES
        self.pipes = []
        self.passed_pipes = 0

    def count_passed_pipes(self):
        for pipe in self.pipes:
            if CACO_POS[0] > pipe.rect.right:
    ...
    def generate_pipes(self):  
        if self.game.caco.initial_jump:
            self.pipe_distance += SCROLL_SPEED
            if self.pipe_distance > DISTANCE_BETWEEN_PIPES:
                self.pipe_distance = 0
                gap_y = self.get_gap_y_pos()
